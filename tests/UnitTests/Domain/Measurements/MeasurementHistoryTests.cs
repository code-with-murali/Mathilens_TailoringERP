...
public class MeasurementHistoryTests
{
    ...
    {
        var measurement = Measurement.Create(Guid.NewGuid(), GarmentType.Trousers, new Dictionary<string, decimal> { ["Waist"] = 34, ["Inseam"] = 30 });

        var snapshot = MeasurementHistory.CaptureSnapshot(measurement);

        Assert.Equal(measurement.Id, snapshot.MeasurementId);
        Assert.Equal(GarmentType.Trousers, snapshot.GarmentType);
    ...
    {
        var measurement = Measurement.Create(Guid.NewGuid(), GarmentType.Trousers, new Dictionary<string, decimal> { ["Waist"] = 34 });
