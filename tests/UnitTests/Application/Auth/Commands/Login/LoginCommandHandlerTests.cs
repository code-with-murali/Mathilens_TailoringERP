...
public class LoginCommandHandlerTests
{
    ...
    public async Task Handle_DelegatesToIdentityServiceWithCommandCredentials()
    {
        ...
        identityService
            .LoginAsync("owner@shop.example", "s3cret!", Arg.Any<CancellationToken>())
            .Returns(Result.Success(expectedTokens));

        var handler = new LoginCommandHandler(identityService);

        var result = await handler.Handle(new LoginCommand("owner@shop.example", "s3cret!"), CancellationToken.None);

        Assert.True(result.IsSuccess);
        Assert.Equal(expectedTokens, result.Value);
        await identityService.Received(1).LoginAsync("owner@shop.example", "s3cret!", Arg.Any<CancellationToken>());
    }
    ...
    {
        var identityService = Substitute.For<IIdentityService>();
        var error = Error.Unauthorized("Auth.InvalidCredentials", "Invalid email or password.");
        identityService
        ...
        var result = await handler.Handle(new LoginCommand("owner@shop.example", "wrong"), CancellationToken.None);
