...
internal static class OrderMapper
{
    public static OrderDto ToDto(this Order order) =>
        new(order.Id, order.CustomerId, order.EmployeeId, order.Status, order.DueAtUtc, order.DeliveredAtUtc, order.Notes, order.CreatedAtUtc, order.Items.Select(i => i.ToDto()).ToList());
